...
from question_answer_pairs import question_answer_pairs
from langchain.evaluation.qa import QAEvalChain
import pandas as pd

# These 5 chunk sizes should be enough to get a good idea of the ideal size
# CHUNK_SIZES_TO_EVAL = [250, 500, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000]
CHUNK_SIZES_TO_EVAL = [1000, 2000, 3000, 4000]#, 5000, 6000, 7000, 8000]
...
def make_splits(chunk_size, overlap, text):
    ...
    db = FAISS.from_texts(splits, NomicEmbeddings(model='nomic-embed-text-v1'))
    ...
    chain = VectorDBQA.from_chain_type(llm=ChatOpenAI(temperature=0, model_name="gpt-4-0125-preview"), 
                                       chain_type="stuff", 
                                       vectorstore=db, 
                                       input_key="question")
...
for i, chunk_size in enumerate(CHUNK_SIZES_TO_EVAL):
    ...
    print(f"Chunk size {chunk_size} has {sum(incorrect_counts)} incorrect answers")
